...
class TreeMatch:
    ...
    def doTreeMatch(self):
        ...
        tmp_mat = self.comm_mat
        for d in range(len(self.top_graph.layers) - 1, 1, -1):
            p = len(tmp_mat)
            if p % self.arity(d - 1) != 0:
                tmp_mat = self.extendCommMatrix(d)
            groups[d] = self.groupProcesses(d)
            tmp_mat = self.aggregateCommMatrix(groups[d])
        return [x for xs in groups[-1] for x in xs]

    def groupProcesses(self, cur_depth):
        ...
        independent_set = G.independent_vertex_sets(len(self.comm_mat) // self.arity(cur_depth - 1), len(self.comm_mat) // self.arity(cur_depth - 1))
        # print("independentset[0] ", independent_set[0], l[independent_set[0][0]], l[independent_set[0][1]])
        # TODO: filter the sets greedy
        return [l[g] for g in independent_set[0]]
    ...
    def extendCommMatrix(self, cur_depth) -> list:
        ...
        ext_mat = self.comm_mat
        for l in ext_mat:
            l.extend([0] * k)
        for _ in range(k):
            ext_mat.append([0 for _ in range(p)])
        return ext_mat
