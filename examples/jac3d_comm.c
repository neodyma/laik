...
int main(int argc, char* argv[])
{
    ...
    while ((argc > arg) && (argv[arg][0] == '-')) {
        ...
        if (argv[arg][1] == 'd' && argc > arg+1) { halo_depth = atoi(argv[++arg]); }
        if (argv[arg][1] == 'h') {
    ...
    Laik_CommMatrix* testmat = laik_top_CommMatrix_init(inst);
    ...
    prRead = use_cornerhalo ? laik_new_cornerhalo_partitioner(halo_depth) :
                              laik_new_halo_partitioner(halo_depth);
    ...
    if (laik_myid(world) == 0)
        printf(":::: jac3d_comm %s (np%d) %d %d g%d\n:::: t %fs\n",
            laik_top_reordering_get(inst) != 0 ? "RO" : "NR",
            laik_size(world),
            size,
            maxiter,
            xblocks,
